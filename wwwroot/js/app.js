...
class App {
    init() {
        ...
        // Add a document-ready handler to ensure dropdowns are initialized after DOM is fully loaded
    ...
    initComponents() {
        ...
        // Dropdowns - Enhanced initialization with error handling
        // Skip header dropdowns as they are handled separately in header-dropdowns.js
        document.querySelectorAll('[data-bs-toggle="dropdown"]:not(.topbar-item .dropdown-toggle):not(.nav-user .dropdown-toggle)').forEach((el) => {
            try {
        ...
        // Special handling for header dropdowns (notification, message, user profile)
        this.initHeaderDropdowns();
    ...
    initHeaderDropdowns() {
        // This function is now handled by the dedicated header-dropdowns.js file
        // We keep this as a placeholder to avoid breaking existing code
        console.log('Header dropdowns are handled by header-dropdowns.js');
    }
    ...
    initLeftSidebar() {
        ...
        // Note: Active menu item handling is now managed by _SideNav.cshtml
        // This prevents conflicts between multiple active menu systems
...
class LayoutCustomizer {
    ...
    initConfig() {
        // Check if window.defaultConfig exists and is valid JSON
        ...
        // Check if window.config exists and is valid JSON
    ...
    initSwitchListener() {
        ...
        // Also add click event to the backdrop to close sidebar
    ...
    showBackdrop() {
        ...
        if (backdrop) {
            // If backdrop already exists, make sure it's visible
        ...
        newBackdrop.id = "custom-backdrop";
